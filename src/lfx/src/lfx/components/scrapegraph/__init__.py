"""
模块名称：ScrapeGraph 组件延迟加载入口

本模块提供 ScrapeGraph 相关组件的按需导入，主要用于避免未安装可选依赖时在导入阶段失败。主要功能包括：
...
设计背景：ScrapeGraph SDK 是可选依赖，懒加载可降低安装与导入失败风险。
使用场景：仅在访问 ScrapeGraph 组件时加载实现。
