"""
模块名称：Not Diamond 组件延迟加载入口

本模块提供 Not Diamond 组件的按需导入，主要用于避免未安装可选依赖时在导入阶段失败。主要功能包括：
...
设计背景：路由组件依赖外部服务，采用懒加载降低安装门槛。
使用场景：仅在访问 Not Diamond 组件时才加载依赖。
