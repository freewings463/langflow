"""
模块名称：Composio Canvas 组件

本模块提供 Composio 平台的 Canvas 接入组件，用于在 Langflow 中调用对应工具。
主要功能包括：
- 绑定 `app_name` 为 `canvas`，以匹配 Composio 工具包标识
- 暴露组件元信息（显示名/图标/文档地址）

关键组件：
- ComposioCanvasAPIComponent：Canvas 的 Composio 组件封装
