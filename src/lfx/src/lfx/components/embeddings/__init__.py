"""
模块名称：embeddings 组件入口

本模块负责 embedding 相关组件的懒加载与导出。
主要功能包括：
- 功能1：按需导入 `EmbeddingSimilarityComponent` 与 `TextEmbedderComponent`。

使用场景：在流程中使用 embedding 相关组件时按需加载。
关键组件：
- 函数 `__getattr__`：属性访问触发懒加载。

设计背景：减少不必要依赖加载并缩短启动时间。
